...
class Iterator(object):
    ...
    def _flow_index(self, N, batch_size=32, shuffle=True, seed=None, infinite=True):
...
class DirectoryIterator_DIV2K(Iterator):
    def __init__(self,
        ...
        import glob
        lrs = glob.glob(datadir+'/LR/*.png')
        lrs.sort()

        sharps = glob.glob(datadir+'/HR/*.png')
        sharps.sort()
    ...
    def next(self):
        ...
        while (len(batch_blur) < self.out_batch_size):
            ...
            try:
                B_ = _load_img_array(blurs,color_mode='Y') 
                S_ = _load_img_array(sharps,color_mode='Y') 
            except:
